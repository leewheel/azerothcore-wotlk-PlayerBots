...
/*static*/ bool Acore::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(ChatHandler& handler, std::string_view cmdStr)
{
    ...
    {
        LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: sScriptMgr->OnTryExecuteCommand consumed the command");
        return true;
    }

    LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: cmd={} cmdStr='{}' oldTail='{}'",
        (void*)cmd, cmdStr, oldTail);

    /* if we matched a command at some point, invoke it */
    if (cmd)
    {
        LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: cmd='{}' IsInvokerVisible={} hasInvoker={}",
            cmd->_name, cmd->IsInvokerVisible(handler), cmd->_invoker.operator bool());

        handler.SetSentErrorMessage(false);
        if (cmd->IsInvokerVisible(handler) && cmd->_invoker(&handler, oldTail))
        { /* invocation succeeded, log this */
            LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: Invoker returned TRUE");
        ...
        {
            LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: Invoker NOT called or returned FALSE. IsInvokerVisible={} HasSentErrorMessage={}",
                cmd->IsInvokerVisible(handler), handler.HasSentErrorMessage());
    ...
    LOG_ERROR("server.worldserver", "[CLI-DIAG] TryExecuteCommand: No command matched");
